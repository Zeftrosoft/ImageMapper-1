...
# pip install pymysql
# pip install pandas
# git clone https://github.com/blaze/odo.git
# Update Source of ODO With Some Changes And Bugfixes, Refer Readme
# cd odo
...
class DBHelper:
    ...
    def getPersonData(self, cid):
        querry = 'SELECT person.*,global_session.email FROM person LEFT JOIN global_session ON  global_session.pid = person.pid where cid={0}'.format(
            cid)
        self.connect()
        try:
    ...
    def getClassEnrollImageData(self, cid, pid):
        querry = "SELECT * FROM person_image where is_Enroll=1 AND cid={0} AND pid={1}".format(
            cid, pid)
        self.connect()
        try:
    ...
    def getOnlyClassEnrollImageData(self, cid):
        querry = "SELECT * FROM person_image where is_Enroll=1 AND cid={0}".format(
            cid)
        self.connect()
        enroll_images = self.cursor.execute(querry)
        enroll_image_list = self.cursor.fetchall()
        if len(enroll_image_list) <= 0:
            enroll_image_data = {
                'status': False,
                'data': [],
                'msg': 'Enroll Image Data Doesnot Exists'
            }
        else:
            enroll_image_data = {
                'status': True,
    ...
    def getClassPredictImageData(self, cid, pid):
        querry = "SELECT * FROM person_image where is_Enroll=0 AND cid={0} AND pid={1}".format(
            cid, pid)
        self.connect()
        try:
    ...
    def getClassPredictImageDataLimited(self, cid, pid):
        querry = "SELECT * FROM person_image where is_Enroll=0 AND cid={0} AND pid={1} AND NOT EXISTS (SELECT * FROM mapper WHERE mapper.prediction_img_path = person_image.image_path)".format(
            cid, pid)
        print(querry)
        self.connect()
        predict_image_data = {}
    ...
    def userExists(self, user, isLogin):
        user_exists = True
        if isLogin:
            querry = "SELECT userId FROM user where email='" + \
                user['email']+"' AND pass='"+user['password']+"'"
        else:
    ...
    def registerUser(self, user):
        ...
        if not res['status']:
            querry = "INSERT INTO user (email, pass) VALUES ('" + \
    ...
    def getMapperData(self, cid, pid, prediction_img_path, enroll_img_path):
        querry = "SELECT * FROM mapper where cid={0} AND pid={1} AND prediction_img_path=\'{2}\'".format(
            cid, pid, prediction_img_path)
        self.connect()
        try:
    ...
    def upsertMapperData(self, mapper):
        ...
        if res['status']:
            ...
            querry = "INSERT into mapper(cid,pid,prediction_img_path,enroll_img_path,isSkipped,isNotEnrolled) VALUES({0},{1},\'{2}\',\'{3}\',{4},{5})".format(
                mapper['cid'], mapper['pid'], mapper['prediction_img_path'], mapper['enroll_img_path'], mapper['isSkipped'], mapper['isNotEnrolled'])
        self.connect()
        try:
            self.cursor.execute(querry)
            self.conn.commit()
    ...
    def getclassid(self, class_name):
        querry = "select cid from class where class_name=\'{0}\'".format(
            class_name.lower())
        self.connect()
        try:
    ...
    def getGlobalSessionData(self, email):
        querry = 'SELECT * FROM global_session where email=\'{0}\''.format(
            email)
        self.connect()
        try:
    ...
    def upsertGlobalSessionData(self, email, pid):
        ...
        if res['status']:
            ...
            querry = "INSERT into global_session(pid, email) VALUES({0},\'{1}\')".format(
                pid, email)
        self.connect()
        try:
            self.cursor.execute(querry)
    ...
    def upsertNmatchData(self, image_path, nmatch_path):
        ...
        if res['status']:
            ...
            querry = "INSERT into nearest_match(image_path, nmatch_path) VALUES(\'{0}\',\'{1}\')".format(
                image_path, nmatch_path)
        self.connect()
        try:
            self.cursor.execute(querry)
            self.conn.commit()
    ...
    def getNmatchData(self, image_path, nmatch_path):
        querry = "SELECT * FROM nearest_match where image_path=\'{0}\' AND nmatch_path=\'{1}\'".format(
            image_path, nmatch_path)
        print(querry)
        self.connect()
        try:
    ...
    def getAllNmatchData(self, pred_path):
        querry = "SELECT * FROM nearest_match where image_path='"+pred_path+"'"
        print(querry)
        self.connect()
        try:
            nmatchs = self.cursor.execute(querry)
    ...
    def getMappedData(self, cid, pid):
        querry = "SELECT * FROM mapper where cid={0} AND pid={1} ".format(
            cid, pid)
        self.connect()
        try:
            mappers = self.cursor.execute(querry)

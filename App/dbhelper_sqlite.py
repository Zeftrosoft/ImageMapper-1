...
class DBHelper:
  ...
  def connect(self):
    ...
    self.conn = sqlite3.connect('./data/mydb.db', check_same_thread=False)#, check_same_thread= False
  ...
  def getOnlyClassEnrollImageData(self, cid):
    querry = "SELECT * FROM person_image where is_Enroll='True' AND cid={0}".format(cid);
    self.connect()
    enroll_images = self.cursor.execute(querry)
    enroll_image_list = list(enroll_images)
    if len(enroll_image_list) <= 0 :
